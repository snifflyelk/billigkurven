    ...
    try {
      ...
      setMessage(
        payload.fallback
          ...
          : channel === "email"
            ? "Ukentlig rapport sendt på epost."
            : "Push-varsling sendt.",
      );
    } catch {
  ...
  return (
    <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
      ...
      <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
        Send denne rapporten nå på epost eller push. Kjede-filter brukes i utsendelsen.
      </p>
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        >
          {isSendingEmail ? "Sender epost..." : "Send epost nå"}
        </button>
        <button
          type="button"
        ...
        >
          {isSendingPush ? "Sender push..." : "Send push nå"}
        </button>
      </div>
